...
public class JoinDialog extends FloatingDialog{
        ...
        for(Server server : servers){
            ...
            TextButton button = buttons[0] = remote.addButton("[accent]" + server.displayIP() , Styles.cleart, () -> {
                if(!buttons[0].childrenPressed()){
                    Manifest.currentServer = server.displayIP();
    ...
    void buildServer(Host host, Table content){
        ...
        content.table(t -> {
            t.add("USID: " + NetClient.getUsid( "/"+ host.address + ":" + host.port));
            t.row();
    ...
    public void connect(String ip, int port){
        Manifest.currentServer = ip + ":" + port;
        gae.reporter.report("Joining");
        //gae.commandCenter.tellSlave("CONNECT " + Manifest.currentServer);
        ...
        Time.runTask(2f, () -> {
            ...
            net.connect(ip, port, () -> {
                hide();
                add.hide();
                gae.commandCenter.slaveConnectToServer(Manifest.currentServer);
            });
        });
    }
